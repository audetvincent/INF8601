...
int create_buffer(int width, int height)
{
    ...
    cl_int ret = 0;
    size_t buffBytes;
    buffBytes = width * height *sizeof(cl_char)*3;

    output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, buffBytes, NULL, &ret);
    ERR_THROW(CL_SUCCESS, ret, "create_buffer failed");
...
int sinoscope_image_opencl(sinoscope_t *ptr)
{
    ...
    size_t work_dim[2] = {sino.width, sino.height};
